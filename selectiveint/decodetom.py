...
# Using Ethan Meyers's version of a correlation classifier with multiple small splits. The spurious anti-correlation (whereby the first stimulus 
# anti-predicts the second) is strongly attenuated, but not removed altogether - presumably a nearest-neighbour classifier would get rid of it.
...
matdata += 1.0 * standard_normal(shape(matdata))
...
# 10 is best w/o noise... except it has a bright spot in the bottom right corner for 1st stimulus decoding!
# 14 is great w/ noise
# Lower split size and larger nb of iters make for smoother graphs
...
# for all 4 conditions (i.e. all 4 possible combinations of inputs):
...
for numgraph in range(4):
    ...
    for numiter in range(100):
        ...
        for numtest in range(testdata.shape[2]):
            ...
            # At every point row,col, find the label / class whose average at time row has the maximum correlation with the test at time col
            guessedclass = argmax(corrsperclass, axis=2)
