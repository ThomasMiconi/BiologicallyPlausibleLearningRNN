...
# This is or the positive-responses, Dal'es law networks. needs to be modified from the normal, signed-responses code. In particular, the "criterion" (which is based on error <1) makes no sense when responses can go to 20. I put criterion 2.5 because target responses are 0 and 5.
...
for selectstr in selectstrs:
    ...
    if 1:
        ...
        for nm in fnames:
            ...
            correctz = uint8(eee<2.5)   # Because target responses are 0 and 5
...
mediancrittimes = [np.median(x) for x in crittimesallplots]
q75crittimes = [np.percentile(x , 75) for x in crittimesallplots]
...
selectstrssortedq75 = [selectstrs[nn] for nn in orderq75]

draw() # why do I have to put this ?... 
